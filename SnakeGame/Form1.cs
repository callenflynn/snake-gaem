...
namespace SnakeGame
{
    public partial class Form1 : Form
    {
        ...
        private void PlayEmbeddedSound()
        {
            string resourceName = "SnakeGame.assets.Blip.wav"; // Confirm this is correct!
            string tempPath = Path.Combine(Path.GetTempPath(), "Blip.wav");
            ...
            try
            {
                using (Stream stream = Assembly.GetExecutingAssembly().GetManifestResourceStream(resourceName))
                {
                    if (stream == null)
                    {
                        MessageBox.Show("Embedded sound not found: " + resourceName);
                        return;
                    }
